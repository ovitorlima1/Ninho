...
function Phone({ children, title, activeRoute, setLocation, activePanel, onPanel }: {
  ...
  return (
    <section className={`phone phone-${title === "Ninho" ? 0 : title === "Quick log" ? 1 : 2} ${activePanel === (title === "Ninho" ? 0 : title === "Quick log" ? 1 : 2) ? "is-mobile-active" : ""}`} aria-label={title}>
  ...
  return (
    <div className="phone-content flow">
      ...
      <div className="focus-card">
        ...
        <div className="orbit-illustration" aria-label="Ilustração abstrata de progresso"><i /><b /><em><span>{done}</span><small>de {items.length}</small></em></div>
      </div>
